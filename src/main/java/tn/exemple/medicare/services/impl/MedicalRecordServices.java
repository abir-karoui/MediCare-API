   ...
   public MedicalRecordResponse getMedicalRecord() {
    ...
    /*public MedicalRecordDTO updateMedicalRecordPartial(MedicalRecordDTO dto) {
        ...
        MedicalRecord medicalRecord = iMedicalRecordRepository.findByPatient(patient)
                .orElseThrow(() -> new EntityNotFoundException("Medical record not found for this patient."));
        ...
        if (dto.getOperations() != null) {
            List<OperationDTO> newOperations = dto.getOperations();
            List<Operation> existingOperations = medicalRecord.getOperations();

            Map<Long, Operation> existingOperationMap = existingOperations.stream()
                    .filter(op -> op.getId() != null)
                    .collect(Collectors.toMap(Operation::getId, op -> op));

            List<Operation> updatedOperations = new ArrayList<>();

            for (OperationDTO opDto : newOperations) {
                if (opDto.getId() != null && existingOperationMap.containsKey(opDto.getId())) {
                    // Mise à jour d'une opération existante
                    Operation existingOp = existingOperationMap.get(opDto.getId());
                    existingOp.setName(opDto.getName());
                    existingOp.setDateOperation(opDto.getDateOperation());
                    existingOp.setSurgeon(opDto.getSurgeon());
                    existingOp.setHospital(opDto.getHospital());
                    existingOp.setDescription(opDto.getDescription());
                    updatedOperations.add(existingOp);
                } else {
                    ...
                    updatedOperations.add(newOp);
                }
            }

            // Détermination des opérations supprimées
            Set<Long> newIds = newOperations.stream()
                    .map(OperationDTO::getId)
                    .filter(Objects::nonNull)
                    .collect(Collectors.toSet());

            List<Operation> toRemove = existingOperations.stream()
                    .filter(op -> op.getId() != null && !newIds.contains(op.getId()))
                    .collect(Collectors.toList());

            existingOperations.removeAll(toRemove); // suppression
            existingOperations.clear();
            existingOperations.addAll(updatedOperations); // mise à jour complète de la liste
        }
